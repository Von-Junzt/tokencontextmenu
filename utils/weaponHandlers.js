...
import {shouldAutoRemoveTargets, isExtractedFeatureLogicEnabled} from "../settings/settings.js";
...
export async function handleWeaponSelection(token, weaponId, hideMenuCallback) {
    // Phase 2: Use new manager if feature flag is enabled
    if (isExtractedFeatureLogicEnabled()) {
        debug("Phase 2: Using TargetingSessionManager for weapon selection");
...
export async function handleWeaponEquip(actor, weaponId) {
    // Phase 2: Use new manager if feature flag is enabled
    if (isExtractedFeatureLogicEnabled()) {
        debug("Phase 2: Using EquipmentModeHandler for weapon equip");
...
export async function handlePowerFavoriteToggle(actor, powerId) {
    // Phase 2: Use new manager if feature flag is enabled
    if (isExtractedFeatureLogicEnabled()) {
        debug("Phase 2: Using EquipmentModeHandler for power favorite toggle");
...
export async function handleWeaponReload(actor, weaponId) {
    // Phase 2: Use new manager if feature flag is enabled
    if (isExtractedFeatureLogicEnabled()) {
        debug("Phase 2: Using EquipmentModeHandler for weapon reload");
...
export async function handleWeaponUnequip(actor, weaponId) {
    // Phase 2: Use new manager if feature flag is enabled
    if (isExtractedFeatureLogicEnabled()) {
        debug("Phase 2: Using EquipmentModeHandler for weapon unequip");
